...
===========================
*** DEPRECATED as of Phase 1 (accuracy fix) -- do not use. ***
Augmenting the WHOLE dataset here, before train_model.py's split, let
near-duplicate augmented copies of the same original sample land in both
the train and test sets. That inflated reported test accuracy without
actually improving real-world accuracy. This step has been folded into
scripts/prepare_dataset.py, which splits into train/val/test FIRST and
only ever augments the train portion. Kept in the repo for reference only.
